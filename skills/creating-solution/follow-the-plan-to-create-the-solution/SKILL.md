...
description: Use when an approved implementation ticket or plan is ready to execute, resume, or reconcile.
---
...
4. Implement one complete vertical slice at a time. Write and run a failing test first when the approved method calls for test-driven development.
...
After delegated work, inspect the actual diff and run verification independently. Treat the subagent report as a lead, not proof. Keep the main agent responsible for integration and durable state.

When verification repeatedly fails, run smaller checks and test falsifiable causes one at a time. Use D1 and D2 to gather evidence when the failure is not yet understood. Reconsider the design after evidence shows that the current shape is the cause.
