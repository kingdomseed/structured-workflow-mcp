---
name: start-evaluate
description: Start or coordinate Criterion D by designing relevant testing methods, gathering fresh evidence, judging work against explicit criteria, deriving improvements, and explaining impact. Use for the final evaluation of a solution or to review a Design Brief, Spec, plan, ticket set, prototype, or other intermediate artifact.
---
...
Criterion D has four callable criterion skills:
...
Evaluation can be entered from any phase. Follow the fog of war and the movement rules in `AGENTS.md`.

## Name the bar before judging
...
Keep independent axes separate when one could mask another. For example, judge Spec coverage separately from project standards, and functional behavior separately from visual fidelity or accessibility.
...
The shipping decision is the design cycle's hard gate. A solution is ready to ship only when every required design specification and Definition of Done criterion has passing evidence. Missing evidence remains unproven or blocked; it does not become a pass.
