...
Goal: Create a `Flavor` enum and a `getFlavor` function to return the current flavor.

### Prerequisites

`pubspec.yaml` already contains a `flavorizr` section defining the flavors for the app. Example:
...
flavorizr:
  ...
  flavors:
    ...
    stg:
      # stg flavor configuration

    prod:
      # prod flavor configuration
```

If this does not exist, abort this command immediately.

### Implementation

Create `lib/src/env/flavor.dart` file if missing, based on the flavors defined in `pubspec.yaml`. Example:

```dart
// lib/src/env/flavor.dart
